...
export const deployContract = async (contractArtifactName: string, constructorArguments?: any[], options?: DeployContractOptions) => {
  ...
  const contract = await deployer.deploy(artifact, constructorArguments, undefined, [artifact.bytecode, ...(options?.bytecodes ?? [])]);
  ...
  return { contract, address, bytecodeHash, bytecode: artifact.bytecode };
}
